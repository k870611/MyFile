...
class ServerCaller():
    ...
    def ipmicall_raw_call(self, rawparams):
        ...
        if errstr != '':
            ...
            raise IPMIException(errstr + ' from ipmitool ' + command)
    ...
    def generate_update_config(self, loadedtable, configfile, stamptime): # configfile, stamptime): #folder, fileprefix, fileext): #(self, loadedtable, folder, fileprefix, fileext):
        ...
        while True:
            ...
            try:
                ...
                if 'Data' not in loadedtable['PRT']:
                    filetype = 3 # PRT
                    loadedtable['PRT']['Data'] = []
                    loadedtable['PRT']['FileType'] = filetype
                    recordsize = self.schemaparser.fileinfo[BIOSFileSchemaParser.FILE_MARKS[filetype]]['RecordSize']
                    for pos in range(0, len(loadedtable['PRT']['Records']), recordsize):
                        self.schemaparser.parse_update_table(loadedtable['PRT'], loadedtable['PRT']['Records'][pos:])
                ...
                if len(errors) > 0:
                    raise Exception('\n'.join(errors) + ' in {0}'.format(targetfile))
...
class BiosConfig:
    ...
    def process(self, host, user, password, updatecfg=''):
        ...
        if not os.path.isfile(adapter.files['CFG_ADD']):
            info('No update file exist.  Stop.')
            return
...
if __name__ == '__main__':
    # arg = ['-H', '172.22.27.187', '-U', 'admin', '-P', 'admin', '-F', 'adminTest1']
    try:
        arg = [str(i).replace("\r", "") for i in arg if str(i).replace("\r", "") != ""]
        if len(arg) >= 0:
            sys.argv = ['BiosConfig.py'] + [str(i) for i in arg]
