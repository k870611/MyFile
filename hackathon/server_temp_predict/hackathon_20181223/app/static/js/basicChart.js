...
! function(){
	...
	basicChart.drawStackbyNVD3 = function(s, data){
		...
		nv.addGraph(function() {
	        ...
	        chart.xAxis.ticks(5).tickFormat(function(d) { return d3.time.format('%Y-%m-%d %H:%M')(new Date(d));});
	        chart.x2Axis.ticks(5).tickFormat(function(d) { return d3.time.format('%Y-%m-%d %H:%M')(new Date(d)); });
	        chart.yAxis.tickFormat(d3.format(',.2f'));
