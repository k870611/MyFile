...
from app.models import User, EventWarning, EventDevice, EventSystem, EventNow, Server, Role, Log
from app import app, db
from app.forms import LoginForm, AccountCreateForm, AccountModifyForm, RoleModifyForm
...
def serverinfo():
    ...
    server = Server.query.order_by(Server.server_slot).all()
    EventCount = CountEventNow()

    return render_template("serverinfo.html", server_list=server, CountEventNow=EventCount)
...
def after_request(response):
    ...
    app.permanent_session_lifetime = date.timedelta(minutes=30)
