...
#    IpmiCommandFormat = " -H {server[1]} -p {server[2]} -U {server[3]} -E {server[5]} exec {planfile} >> {resultfile} 2>&1"
    IpmiCommandFormat = " -H {server[1]} -p {server[2]} -U {server[3]} -P admin -E {server[5]} exec {planfile} >> {resultfile} 2>&1"
    ...
    def get_servers(self):
        return [
            [1, '192.168.16.202', '623', 'root', 'root', '-I lanplus', 2],
            [2, '192.168.16.201', '623', 'admin', 'admin', '-I lanplus', 10]
        ]
    ...
    def _has_value(self, servermap, serverid):
    ...
    def composable(self, obj, serverid = None):
    ...
    def Dispatcher(self, loop, obj=None):
        ...
        for task in self.tasks:
            ...
            if serverid != task[self.ID]:
                if self.composable(obj, serverid):
                    self.submit(obj)
